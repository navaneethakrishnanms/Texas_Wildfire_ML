...
tune_tx_hrrr.py
-----------------
Hyperparameter tuning for the HRRR-enriched Texas Wildfire XGBoost model.

Same search as tune_tx.py (Stage 1: depth x min_child_weight, Stage 2:
learning_rate x subsample) but run against data/hrrr/hrrr_tx_all.parquet
with the full 42-feature set (gridMET + LANDFIRE + HRRR), instead of
reusing the gridMET-only tuned config from tune_tx.py.

Baseline to beat: xgb_tx_hrrr_meta.json TEST AUROC=0.8682 (untuned-for-HRRR
config borrowed from the gridMET-only tune).

Usage:
    python tune_tx_hrrr.py

Output:
    outputs/texas_landfire/models/xgb_tx_hrrr_tuned.ubj
    outputs/texas_landfire/models/xgb_tx_hrrr_tuned_meta.json
    outputs/texas_landfire/tuning_results_hrrr.csv
"""
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(OUT_DIR / "tune_tx_hrrr.log", encoding="utf-8"),
    ],
)
log = logging.getLogger(__name__)

# ── Baseline to beat (untuned-for-HRRR config, from train_tx_hrrr.py) ─────────
BASELINE_AUROC = 0.8682
BASELINE_AUPR  = 0.4236

# ── Feature columns (must match train_tx_hrrr.py) ─────────────────────────────
BASE_FEATURE_COLS = [
...
FEATURE_COLS = BASE_FEATURE_COLS + HRRR_FEATURE_COLS


# ── Load data ─────────────────────────────────────────────────────────────────
def load_data():
    log.info(f"Loading {HRRR_PQ.name}...")
    ...
    if absent:
        log.warning(f"  Features not found in parquet: {absent}")
...
def tune():
    ...
    log.info(f"  Baseline (borrowed gridMET config): val-AUROC~{BASELINE_AUROC:.4f}")
    log.info(f"{'='*75}")
    ...
    log.info(f"  val AUROC        = {best['val_auroc']:.4f}  "
             f"(vs borrowed-config baseline {BASELINE_AUROC:.4f}, "
             f"Δ={best['val_auroc']-BASELINE_AUROC:+.4f})")
    log.info(f"  val AUPR         = {best['val_aupr']:.4f}  "
             f"(vs borrowed-config baseline {BASELINE_AUPR:.4f}, "
    ...
    log.info(f"\n{'─'*75}")
    log.info("FINAL EVALUATION (tuned HRRR model)")
    log.info(f"{'─'*75}")
    final_metrics = []
    ...
    log.info(f"\n  TOP 15 FEATURES (tuned HRRR model):")
    for feat, gain in imp_sorted[:15]:
        tag = "  [HRRR]" if feat in HRRR_FEATURE_COLS else ""
        bar = chr(9608) * int(gain/imp_sorted[0][1]*25)
        log.info(f"  {feat:<20} {gain:>9.1f}  ({100*gain/total_gain:4.1f}%)  {bar}{tag}")

    hrrr_gain_share = sum(g for f, g in imp_sorted if f in HRRR_FEATURE_COLS) / total_gain
    log.info(f"\n  HRRR features share of total gain: {100*hrrr_gain_share:.1f}%")

    model_path = OUT_DIR / "models" / "xgb_tx_hrrr_tuned.ubj"
    meta_path  = OUT_DIR / "models" / "xgb_tx_hrrr_tuned_meta.json"
    csv_path   = OUT_DIR / "tuning_results_hrrr.csv"
    ...
    meta = {
        "tuning_baseline": {"auroc": BASELINE_AUROC, "aupr": BASELINE_AUPR,
                            "note": "xgb_tx_hrrr (borrowed gridMET-tuned config)"},
        "best_config": {
            "max_depth":        best["max_depth"],
            "min_child_weight": best["min_child_weight"],
            "learning_rate":    best["learning_rate"],
    ...
    if test_m["auroc"] > 0.99:
        ...
        log.info("  TEST AUROC improved — tuned HRRR model beats the borrowed-config version")
    else:
        log.warning("  TEST AUROC did not improve — keep xgb_tx_tuned (gridMET-only)")
